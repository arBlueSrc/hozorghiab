...
class Select extends DataBase //برای ارث بری استفاده می شود extends&&&  نام کلاس والد می باشدDataBase
{
	...
	public function getAssignedCourseToAdmin($user_id, $DESC = 'DESC', $start = 0, $end = 300)
	{
		parent::__construct();

		$sql = $this->_SELECT . " `assign_course_to_admin`";
		$sql .= " WHERE `user_id`=" . $user_id; //در صورتی که مخالف خالی بود به متغیر هم نام قبلی می چسباند
		$sql .= " ORDER BY `id` " . $DESC . " LIMIT " . $start . "," . $end . "";
	...
	public function adminList($DESC = '', $start = 0, $end = 50, $w = '')
	{
		parent::__construct();
